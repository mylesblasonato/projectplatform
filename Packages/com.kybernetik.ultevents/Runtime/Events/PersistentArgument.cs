...
namespace UltEvents
{
    ...
    public sealed class PersistentArgument
    {
        ...
        /// <summary>The <see cref="System.Type"/> of this argument.</summary>
        public Type SystemType
        {
            ...
            internal set
            {
                ...
                Type = GetArgumentType(value, out var assemblyQualifiedName, out var linkIndex);
        ...
        private void AssertType(PersistentArgumentType type)
        {
            if (_Type != type)
                throw new InvalidOperationException(
                    $"{Names.PersistentArgument.Full.Type} is {_Type} but should be {type}");
        }
        ...
        /// </summary>
        public static PersistentArgumentType GetArgumentType(
            Type type,
...
#if UNITY_EDITOR
                if (TryGetLinkable != null &&
                    TryGetLinkable(type, out linkIndex, out var linkType))
                    return linkType;
#endif
